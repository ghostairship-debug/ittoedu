...
test('copies image and cw object through real Flow and Markdown UI, reopens and undoes the return paste once', async ({}, testInfo) => {
  ...
  try {
    ...
    await page.locator('.lesson-directory-tree').getByRole('button', { name: '跨正文复制课例', exact: true }).click()
    ...
    if (!await page.locator('.lesson-directory-tree').getByRole('button', { name: 'copy.md', exact: true }).isVisible())
      await page.locator('.lesson-directory-tree').getByRole('button', { name: '跨正文复制课例', exact: true }).click()
