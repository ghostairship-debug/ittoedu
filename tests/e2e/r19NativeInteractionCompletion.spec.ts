...
test('U03-real-player-navigation checks completion, wrong destination and cancellation in Chromium', async () => {
  ...
  try {
    ...
    const facts = await page.evaluate(async () => {
      ...
      const { addCourseScene } = await load('/src/core/tools/courseLocations.ts') as typeof import('../../src/core/tools/courseLocations')
      const { createTextNode } = await load('/src/core/tools/nativeNodeFactories.ts') as typeof import('../../src/core/tools/nativeNodeFactories')
...
test('global Native click enumeration checks Slide to Flow and rejects a wrong destination in Chromium', async () => {
  ...
  try {
    ...
    const facts = await page.evaluate(async () => {
      const load = (path: string) => import(path)
      const { createBlankCourseProject } = await load('/src/core/course/createCourseProject.ts') as typeof import('../../src/core/course/createCourseProject')
      const { addCourseFlowPage } = await load('/src/core/tools/courseLocations.ts') as typeof import('../../src/core/tools/courseLocations')
      const { createTextNode } = await load('/src/core/tools/nativeNodeFactories.ts') as typeof import('../../src/core/tools/nativeNodeFactories')
      const { sceneNodeToCourseLayerItem } = await load('/src/shared/courseProjectModel.ts') as typeof import('../../src/shared/courseProjectModel')
