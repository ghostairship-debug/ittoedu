...
test('r19 lesson: real PDF DOCX PPTX reading, document conflict and reopen, first save and Save As isolation', async ({}, testInfo) => {
  ...
  try {
    ...
    await page.getByRole('textbox', { name: '课例名称' }).fill('串联电路')
    await page.getByRole('button', { name: '创建课例', exact: true }).click()
    await expect(page.locator('.lesson-workspace-lessons').getByRole('button', { name: /串联电路/ })).toBeVisible()
    ...
    const ref = { lessonId: target.lessonId, lessonDirectory: target.rootPath, relativePath: '01-teaching-plan.md' }
    ...
    await page.getByRole('button', { name: '工作空间根目录 · 刷新' }).click()
    await page.locator('.lesson-directory-tree').getByRole('button', { name: /串联电路/ }).click()
    ...
    // Reactivate the lesson through its real GUI after renderer reload.
    await choose(app, workspace)
    await page.getByRole('button', { name: '打开工作空间', exact: true }).first().click()
    await page.locator('.lesson-workspace-lessons').getByRole('button', { name: /串联电路/ }).click()
    ...
    if (await page.getByRole('button', { name: '专业', exact: true }).getAttribute('aria-pressed') !== 'true') await page.getByRole('button', { name: '专业', exact: true }).click()
