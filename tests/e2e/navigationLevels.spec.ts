...
test('Mixed scene and step semantics survive archive and current-surface try-run in Electron', async () => {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      ...
      flow.blocks.push({ id: 'second-anchor', type: 'paragraph', text: '第二锚点' })
      ...
      const bytes = createCourseProjectArchive({ project, assetFiles: {}, componentFiles: {} })
      ...
      for (const kind of ['flow-block','spatial-camera']) {
        ...
        const host = await (kind === 'flow-block' ? mountFlowLocationTryRun : mountSpatialLocationTryRun)({ container, project, locationId: initial.id })
        container.querySelector<HTMLButtonElement>('[aria-label="展开教师控制器"]')?.click()
        const button = [...container.querySelectorAll<HTMLButtonElement>('button')].find(b => b.textContent === '下一步')!
        button.click()
