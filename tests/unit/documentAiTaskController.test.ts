...
function fixture() {
 ...
 const session = { ref, epoch: 1, getSnapshot: () => ({ source: '原稿', disk: { source: '原稿', version }, dirty: false }), prepareAiEdit: vi.fn(async (ranges, epoch) => { steps.push('prepare'); return { status: 'ready', document: { version, source: '原稿' }, ranges, epoch } }), registerAiTaskStop: vi.fn(() => unregister), previewAiEdit: vi.fn(), applyAiEdit: apply, invalidatePreparedAiEdits: invalidate }
...
it('flushes and prepares before native start, and applies to the pinned session after tab changes', async () => {
 ...
 await controller.start({ name: 'a.md', scope: 'document', getEditor: () => current }, 'codex', '改稿')
 current = null
...
it('Stop during native start invalidates prepared edits and cancels late task without applying', async () => {
 ...
 const started = controller.start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿')
...
it('failed native start releases the stop registration and prepared baseline', async () => {
 ...
 const controller = new DocumentAiTaskController(api, workspace, vi.fn())
 await expect(controller.start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿')).rejects.toThrow('启动失败')
...
it('reports the pinned file and actual saved version to the stage owner only after application', async () => {
 ...
 await new DocumentAiTaskController(api, workspace, vi.fn()).start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿', onApplied)
...
it('starts a directory file edit without converting the ref into a lesson', async () => {
 ...
 await new DocumentAiTaskController(api, workspace, vi.fn()).start({ name: 'notes.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改目录稿')
...
it('keeps partial application suggestions without completing the stage repair', async () => {
 ...
 const api = vi.fn<LessonDocumentAiAPI>(async request => request.operation === 'start'
  ? { taskId: 'task', sessionId: 'native', status: 'running', message: '运行中' }
  : { taskId: 'task', sessionId: 'native', status: 'candidate', message: '候选', apply: { baseVersion: version, epoch: 1, operationId: 'task', edits: [] } })
 await new DocumentAiTaskController(api, workspace, notify).start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '修稿', onApplied)
 await vi.advanceTimersByTimeAsync(1000)
 expect(f.apply).toHaveBeenCalledTimes(1)
 expect(notify).toHaveBeenCalledWith('已保存无冲突修改，1 处建议待处理', 'completed')
 expect(onApplied).not.toHaveBeenCalled()
})
it.each(['flush', 'prepare'] as const)('releases the stop registration when %s throws', async failureAt => {
 const f = fixture()
 if (failureAt === 'flush') f.editor.flush = vi.fn(async () => { throw new Error('flush failed') })
 else f.session.prepareAiEdit.mockRejectedValueOnce(new Error('prepare failed'))
 const notify = vi.fn(), controller = new DocumentAiTaskController(vi.fn(), workspace, notify)
 await expect(controller.start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿')).rejects.toThrow(`${failureAt} failed`)
 expect(f.invalidate).toHaveBeenCalledTimes(1)
 expect(f.unregister).toHaveBeenCalledTimes(1)
 expect(notify).toHaveBeenCalledWith(`${failureAt} failed`, 'failed')
})
it('stops a preparation race without starting the native task', async () => {
 const f = fixture()
 let release!: (value: Awaited<ReturnType<typeof f.session.prepareAiEdit>>) => void
 f.session.prepareAiEdit.mockImplementationOnce(() => new Promise(resolve => { release = resolve }))
 const api = vi.fn<LessonDocumentAiAPI>(), controller = new DocumentAiTaskController(api, workspace, vi.fn())
 const started = controller.start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿')
 await vi.waitFor(() => expect(release).toBeTypeOf('function'))
 await controller.stop()
 release({ status: 'ready', document: { source: '原稿', version }, ranges: [], epoch: 2 })
 await started
 expect(api).not.toHaveBeenCalledWith(expect.objectContaining({ operation: 'start' }))
 expect(f.unregister).toHaveBeenCalledTimes(1)
})
it('reports preview as terminal for chat busy and explicitly stops the retained candidate', async () => {
 vi.useFakeTimers()
 const f = fixture(), notify = vi.fn()
 let settle: (() => void) | undefined
 f.session.previewAiEdit.mockImplementation((_request, onSettled) => { settle = onSettled })
 f.invalidate.mockImplementation(async () => { settle?.() })
 const api = vi.fn<LessonDocumentAiAPI>(async request => request.operation === 'start'
  ? { taskId: 'task', sessionId: 'native', status: 'running', message: '运行中' }
  : request.operation === 'read'
    ? { taskId: 'task', sessionId: 'native', status: 'candidate', message: '候选', apply: { baseVersion: version, epoch: 1, operationId: 'task', edits: [] } }
    : { taskId: 'task', sessionId: 'native', status: 'stopped', message: '已停止' })
 const controller = new DocumentAiTaskController(api, workspace, notify)
 await controller.start({ name: 'a.md', scope: 'document', applyPolicy: 'preview', getEditor: () => f.editor }, 'codex', '改稿')
 await vi.advanceTimersByTimeAsync(1000)
 expect(notify).toHaveBeenCalledWith('AI 改动已准备好，请在文档中查看后应用。', 'preview')
 expect(f.unregister).not.toHaveBeenCalled()
 await controller.stop()
 expect(f.invalidate).toHaveBeenCalled()
 expect(f.unregister).toHaveBeenCalledTimes(1)
 expect(notify).toHaveBeenLastCalledWith('已停止', 'stopped')
})
it('unregisters even when the native Stop request fails', async () => {
 const f = fixture()
 const api = vi.fn<LessonDocumentAiAPI>(async request => {
  if (request.operation === 'start') return { taskId: 'task', sessionId: 'native', status: 'running', message: '运行中' }
  throw new Error('stop failed')
 })
 const notify = vi.fn(), controller = new DocumentAiTaskController(api, workspace, notify)
 await controller.start({ name: 'a.md', scope: 'document', getEditor: () => f.editor }, 'codex', '改稿')
 await expect(controller.stop()).rejects.toThrow('stop failed')
 expect(f.unregister).toHaveBeenCalledTimes(1)
 expect(notify).toHaveBeenLastCalledWith('stop failed', 'failed')
})
