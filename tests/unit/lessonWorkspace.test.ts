...
describe('real lesson ownership', () => {
 ...
 it('first save retains conversation and external Save As cannot escape relative manifest', async () => {
  const f = await fixture(), lesson = await f.service.create(f.workspace, '课例')
  const c = await f.conversations.create({ kind: 'lesson', lesson: lesson.identity }); const file = path.join(lesson.identity.normalizedDirectory, 'course.h5lesson')
  ...
  const bound = await f.conversations.bindFirstProject({ kind: 'lesson', lesson: lesson.identity }, c.conversationId, target)
  expect(bound.conversationId).toBe(c.conversationId); expect(bound.epoch).toBe(1)
  await expect(f.conversations.attachSession({ kind: 'lesson', lesson: lesson.identity }, c.conversationId, randomUUID(), 0)).rejects.toThrow('失效')
  ...
  await expect(f.conversations.bindFirstProject({ kind: 'lesson', lesson: lesson.identity }, c.conversationId, createWorkspaceIdentity('project', outside))).rejects.toThrow('新对话')
 })
 it('isolates malformed records and deletes only records after epoch invalidation and stop', async () => {
  const f = await fixture(), lesson = await f.service.create(f.workspace, '课例')
  const c = await f.conversations.create({ kind: 'lesson', lesson: lesson.identity }), session = randomUUID()
  await f.conversations.attachSession({ kind: 'lesson', lesson: lesson.identity }, c.conversationId, session, 0)
  ...
  const listed = await f.conversations.list({ kind: 'lesson', lesson: lesson.identity }); expect(listed.records).toHaveLength(1); expect(listed.damaged).toHaveLength(1)
  const draft = path.join(f.userData, 'recovery.md'); await fs.writeFile(draft, 'unsaved')
  await f.conversations.delete({ kind: 'lesson', lesson: lesson.identity }, c.conversationId, async ids => {
  ...
  expect((await f.conversations.list({ kind: 'lesson', lesson: lesson.identity })).records).toHaveLength(0)
 ...
 it('reassociates moved lessons and refuses identical live copies', async () => {
  const f = await fixture(), lesson = await f.service.create(f.workspace, '课例'), c = await f.conversations.create({ kind: 'lesson', lesson: lesson.identity })
  ...
  expect((await f.conversations.list({ kind: 'lesson', lesson: reopened.identity })).records[0]!.conversationId).toBe(c.conversationId)
 })
})
