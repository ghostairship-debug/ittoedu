...
it('keeps the manifest and conversation on rejected rebind, while first save and Save As bind their actual targets', async () => {
  ...
  expect((await repository.list({ kind: 'lesson', lesson: lesson.identity })).records.find(item => item.conversationId === conversation.conversationId)!.projectTarget).toEqual(first.conversation!.projectTarget)
