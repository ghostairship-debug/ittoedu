...
describe('structured candidate preparation failure', () => {
  it('retains step diagnostics, resource identities and earlier plus failed behavior evidence with zero live writes', async () => {
    ...
    const candidateId = crypto.randomUUID(), candidate = { version: 1, requestId: request.requestId, candidateId, summary: '更新参数', steps: ['first', 'failed'].map(id => ({ id, tool: 'component.configure', carrier: 'existing-component', lowerCarrierReason: '修改现有互动参数', destination: request.destinations[0], input: { operation: 'update', packageId: 'input-package', staticFallback: { assetId: 'input-asset' } } })) }
