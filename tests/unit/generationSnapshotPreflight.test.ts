...
beforeEach(() => { vi.clearAllMocks(); h.inspect.mockReset().mockResolvedValue({ status: 'ready', width: 1, height: 1 }) })
...
function fixture() {
  ...
  const input = { workspace: { version: 1 as const, projectId: document.id, normalizedPath: 'c:/preflight.h5lesson' },
    scope: 'selection' as const, instruction: '把图片改绿', purpose: 'local-edit' as const, intent: 'edit' as const, prepareDrafts: false }
  return { bridge, input }
}

describe('generation snapshot source-preflight lifecycle', () => {
  it.each(['initial-revision', 'disposed'] as const)('rejects %s changes while source inspection is pending', async phase => {
    ...
    expect((await result).message).toContain(phase === 'disposed' ? 'stale：任务已停止或重新开始' : 'stale：课件内容已改变')
    bridge.dispose()
  })
  it('rejects an externally replaced resource sidecar before starting feedback source inspection', async () => {
    const { bridge, input } = fixture()
    const previous = await bridge.capture(input)
    h.state.assetFiles = { red: new Uint8Array([2]) }
    h.inspect.mockClear()
    await expect(bridge.captureNext(previous)).rejects.toThrow('stale：课件资源已改变')
