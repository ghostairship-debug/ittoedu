...
describe('new lesson conversation current file target', () => {
  it('opens a saved independent copy with exactly one target-bound conversation and reuses it on reopen', async () => {
    ...
    expect((await repository.list(opened.lesson!.identity)).records).toHaveLength(1)
    ...
    expect((await repository.list(lesson.identity)).records).toHaveLength(1)
  })
  ...
  it('reads the current strict V9 file after first save and Save As instead of guessing a historical conversation target', async () => {
    ...
    await repository.create(lesson.identity, '历史旧目标', { version: 1, projectId: 'unrelated-old-project', normalizedPath: firstPath.replace(/\\/g, '/').toLowerCase() })
    ...
    expect((await repository.list(lesson.identity)).records.find(record => record.conversationId === result.conversation!.conversationId)?.projectTarget?.projectId).toBe(first.project.id)
