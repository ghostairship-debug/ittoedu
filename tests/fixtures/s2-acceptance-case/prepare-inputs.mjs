...
try {
  ...
  const results = []
  for (const filename of ['可编辑分数.pptx', '应拒绝的表格.pptx']) {
    const result = await page.evaluate(async (bytes) => {
      const { parsePptxImport } = await import('/src/renderer/project/pptxImport.ts')
      try { const draft = await parsePptxImport(new Uint8Array(bytes)); return { ok: true, pages: draft.slides.length, items: draft.slides.flatMap(s => s.items.map(i => i.content.nativeType)), assets: draft.assets.length } }
