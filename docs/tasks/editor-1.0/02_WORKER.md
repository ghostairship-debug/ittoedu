...
已合入的 T/P/Q/F 以 `origin/main` 为祖先。不要再领取 F1–F3。车道 G 从任务卡写明的 `origin/cursor/flow-near-word-g-0ab9` 分树。
...
3. 从任务卡写明的基线建分支。  
   ...
   - 历史车道 Q（Q1–Q8）：不要再领取。  
   - 历史车道 F（F1–F3）：不要再领取。  
   - 车道 G：从任务卡基线建 `cursor/<task-slug>-0ab9`（全小写，前缀 `cursor/`，后缀 `-0ab9`）。  
