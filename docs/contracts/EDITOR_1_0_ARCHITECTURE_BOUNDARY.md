...
> 权威实现以源码实现为准。
>
> 2026-09-07范围提示：这是1.0时期的边界说明，当前长期权威为[架构合同](../development-plan/ARCHITECTURE_CONTRACT.md)。下文旧AI不可见说明已被1.8默认显示的Owner决定取代，不能作为当前接线依据。§1.3的Flow政策已按Owner批准的[三表面整合方案D1](../development-plan/THREE_SURFACE_ARCHITECTURE_INTEGRATION_PLAN.md#d-v9-适配结论与d1决定)更新为响应式文档统一尺度；不改变持久frame数值，Slide/Spatial既有页/HUD边界继续保留。
...
### 1.3 表面基准布局与运行观察
- Slide编辑状态、当前位置试运行与整课预览使用 **1280×720（16:9）** 逻辑舞台，以letterbox落入可用区域。
- 整课预览使用全屏 16:9 覆盖层，不使用更矮的对话框作为“摄像机”。
- Spatial 世界镜头仍在该孔内平移/缩放；HUD / 教师控制器使用与播放相同的 1280×720 视口坐标。
- Flow基准正文与浮层均使用1逻辑单位=1CSS px，正文按实际宽度响应式换行；viewport项位于实际文档视口，paper项相对纸张原点并随正文滚动。主动运行缩放在排版结果上共同应用，不反向改字号或排版宽度；控制器保持独立可达。旧Flow浮层投影变化须复核，不改存储frame。
