...
- 首次候选正确：无补提示、换模型、人工修参数或宿主拒绝后修正；候选前原生工具内部纠错单列，不能隐藏。简单纵切的快捷出口要求自然指令首次经正式操作完成；一次文件路径成功只证明该路径结果，不能替代这一出口。
...
- 49/60 和必过核心继续按原代表集规则；QP/AG 是机制验证及受影响补充，不另作分母稀释失败。合理开放创作另报，不能补入前两层首次覆盖；完整 B2–B4 仍待相应版本交付。
