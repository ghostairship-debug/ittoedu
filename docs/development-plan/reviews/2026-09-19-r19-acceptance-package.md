...
## ⑪ Owner 授权的实机观察（2026-09-20；最终自动化与台账门禁随后复核）

Owner 已明确授权在其不在电脑旁时，以 Computer Use 完成原本需要 Owner 到场的本机观察。以下均发生在真实 Electron 窗口「互动课件编辑器」，不是 Playwright 后台浏览器。

### PM-01：通过

- 进入 `C:\Users\74755\Documents\测试工作空间` 后，工作台显示资源管理器、真实目录树、项目与会话、工作空间会话、聊天和可展开的布局面板；布局面板含内容区方向、收起两侧栏、展开内容区与恢复默认布局。
- 切换测试项目后，出现「课例工作台」和「新建课件」标签；真实编辑器显示新建、打开、导入 PPT、保存、另存为、撤销/重做、场景状态、课件画布及页面与图层入口。
- 打开「属性与素材」后，面板实际显示元素、组件、图层、属性、互动与动画、开发等入口。新建的验收课件保持「尚未保存」；没有保存、导出、覆盖或丢弃用户文件。
- 因此 PM-01 按 1.9 的「工作台轻改 + 编辑器模式，取消简洁/专业分档」口径签收；矩阵与保全基线已同步为该新口径。

### IME：通过

- Windows Microsoft Pinyin 的真实 `CandidateWindow` 在聊天输入框中可见；普通拼音选字得到「你好」，无丢字、重复或错位。
- `/` 命令菜单打开且拼音候选窗存在时按 Enter，系统输入法将组合态处理为 `/n`；聊天记录没有新增消息，菜单动作没有被接受。此处按输入法自己的设置落入原始拼音是正常系统行为，不是产品误发或误选。
