...
| --- | --- | --- | --- | --- | --- |
| `r15-000-material-contract` | 定义本地材料记录、工程关联、引用与删除语义 | `r14-060-release` | 否 | `contracts-schema`, `main-preload` | 定义共享 `WorkspaceIdentityV1 = projectId + normalizedPath`，版本化本地记录严格解析并按该身份关联；材料原文、索引和 trace 不进 CourseProject/Published/导出；删除行为与课程可见引用保留策略可判定；后续 AI session 必须复用该 identity，不再定义第二套 workspace key |
...
| `r15-020-material-tools-citations` | Authoring Tools 可引用材料并写入可见、可携带来源 | `r15-010-material-repository` | 否 | `main-preload`, `editor-store-history` | 从检索结果插入内容时，正文和来源定位写成普通 V9 内容；删除本地材料后已写引用在保存重开、Player 和导出中仍可见；stale target 零写入；引用插入可一次 Undo |
| `r15-030-pptx-import` | 受限 PPTX 解析、资源限制、不支持项报告与 document + sidecar 原子提交 | `r14-060-release` | 否 | `app-save-recovery`, `editor-store-history` | 正常 fixture 导入为可编辑页面 / 对象；超文件大小、超解压比、损坏关系和不支持对象分别停止并报告页码 / 类型；任一 document 或 sidecar 写入失败时两者均不提交；导入可整体撤销；本节点在现有 `tests/unit/courseProjectArchive.test.ts` 增加并通过 PPTX document + sidecar 原子事务用例 |
| `r15-040-style-remix` | 复制参考页可编辑骨架并用明确槽位替换内容 | `r13-060-release`, `r14-060-release` | 否 | `editor-store-history`, `generated-index` | Remix 预览列出骨架来源、槽位映射与容量处理；确认后创建无身份冲突的普通 V9 内容；修改副本不影响来源；缺槽位 / 超容量返回定位结果且不留下半页；保存重开与 Player 一致 |
| `r15-050-content-qa` | 数学、答案一致性、图表正文一致性和来源定位进入可导航 QA | `r14-060-release` | 否 | `generated-index` | 四类预置错误分别产生可跳转 finding；数学检查区分解析错误与渲染警告，答案检查比较题目与 evaluator，图表检查比较数据与正文主张，来源检查定位缺失引用；检查只读且不因 OpenMAIC 缺席降级；本节点在现有 `tests/unit/courseProjectHealth.test.ts` 增加并通过四类内容 QA 用例 |
| `r15-060-release` | 固定课例通过材料、PPTX、Remix 与内容 QA 人工闭环并发布 1.5 源码 tag | `r15-020-material-tools-citations`, `r15-030-pptx-import`, `r15-040-style-remix`, `r15-050-content-qa` | 否 | `none` | Owner 在固定课例导入/检索/引用/删除材料，导入 PPTX，完成 Remix，修复四类 QA finding，并检查保存重开、Player 与适用导出；把已验收行为晋升到保全矩阵，证明 WorkspaceIdentity 单一、材料 repository 不成为第二工程真相、PPTX 复用 lifecycle/transaction port 后签署 `accepted`；OpenMAIC 缺席不阻塞，发布源码 tag |
...
- 本地材料目录使用版本化 metadata + content / index，owner 至少含工程 ID 与规范化文件位置；材料缓存、搜索索引、解析 trace 均不写入 `.h5lesson`。
...
版本候选再执行总路线的统一验证与发布门；OpenMAIC 测试不得加入核心命令链。
