...
已有原生预检覆盖图像、问答、取消和续轮的相应范围，含失败与模型差异，见[预完成证据](../../reviews/1.8-first-batch-preflight.md)。2026-09-09后续工作按[当前执行包](FIRST_BATCH_EXECUTION.md)修已核实剩余缺口，不重新启动已集成的四项旧修复；未变证据继续复用。旧只读/白名单/拒绝授权配置不证明完整CLI能力对等，090–094修订实际能力配置后只补受影响的原生行为证据，不重跑无关付费探针。

早期仅文本、一次关闭stdin和无读取的接线已经出现后续实现，当前反例以[统一方案](../../../../AI编辑最短路径产品决策报告.md)的9月9日核验和实际源码为准，9月8日审查保留历史来源；不可恢复成旧实现边界。官方文档、协议探针和当前GUI行为各自证明相应范围。
