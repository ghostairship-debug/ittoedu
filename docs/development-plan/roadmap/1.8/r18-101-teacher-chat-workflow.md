...
本节点在现有Chat交付当前任务必要操作：意图、真实模型/强度、引用目标/同步状态、发送/停止、提问与原生授权、未应用/已应用/部分完成/失败、变更审阅与Undo。以下行为全部完成才关闭101，不等待全面工作台重设计。聊天首页、项目/会话布局、首个产物展开画布、极简局部AI/专业模式及未命名/双流程归1.9的041/042/044，不前置到本节点。
...
```text
npx --no-install vitest run tests/unit/formulaNodeUi.test.tsx tests/unit/electronLaunchEnvironment.test.ts
npx --no-install playwright test tests/e2e/stabilizationCoreUsability.spec.ts --grep "S3 默认可见与普通讨论：安全消息、分页事件重放及零工程写入|S3 聊天失败注入：一次修复、无进展停止、取消与人工撤销后旧结果零写入"
```

用一种主力CLI在真实窗口按讨论→计划→执行→必要授权→中途纠正→查看变更→Undo走完；允许/拒绝/取消的确定性事件覆盖与adapter共享，不重复付费链。键盘可达关键操作，至少30个本地原生事件测UI投影p95≤300ms。三CLI不同配置/提问/纠正/授权能力的完整工作流由103汇合；不能把主力结果代签其他CLI。
