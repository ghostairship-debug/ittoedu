...
| `r16-040-release` | 形成 1.6 engineering candidate 并发布 v1.6.0-rc.N 源码标签 | `r16-031-hidden-manual-isolation`, `r15-060-release` | 否 | `none` | 自动化与 1.6 全部目标测试通过，三种真实 CLI 分别完成 probe、start、stream、resume、cancel，并在不可用状态下证明人工闭环不退化；普通构建无入口、dogfood 可显式启用，且无第二 session writer 或跨 workspace 泄漏后创建 `v1.6.0-rc.N` 源码标签；本节点不签署 accepted，保全矩阵晋升留到 S3 |
