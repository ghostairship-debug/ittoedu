...
|---|---|---|---|
| PM-01 | 桌面核心 UI 可启动；工作台可切换真实工作空间、浏览资源/项目/会话、创建或打开课件并调整布局；编辑器模式保留场景、画布、页面与图层、属性与素材及 DeveloperTab 入口；取消简洁/专业二档。 | `npm run test -- tests/unit/LessonWorkspaceShell.test.tsx tests/unit/simpleEditorMode.test.tsx tests/unit/developerMode.test.tsx tests/unit/editorActionRouting.test.ts`；涉及真实桌面 wiring 时复用或运行 `npm run test:e2e -- tests/e2e/stabilizationCoreUsability.spec.ts`，并由 Owner 在真实桌面确认主要入口可见且可操作。 | 不得以聊天或只读预览替代人工编辑入口；不得隐藏场景/属性/DeveloperTab，或重新引入简洁/专业双模式；不得把编辑器入口变成无动作占位。 |
