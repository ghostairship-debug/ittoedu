...
044持有唯一阶段、确认、首成果事实及其UI接线，使用041已完成的导航/标签容器和049真实文件接口；041不再反向等待044整项完成。共享App/Chat/IPC由唯一UI集成人顺序修改，不各建状态；实际写锁只覆盖当批文件。目标合同见[课例与文件合同](../../R19_LESSON_DOCUMENT_WORKSPACE_CONTRACT.md)，本次不保留旧阶段制品或会话格式的兼容迁移。

## 执行步骤

1. 在唯一任务合同中区分自动/手动流程、讨论/计划/编辑意图和应用策略。教学简报、策划、呈现简报、脚本四份正文保存在课例真实Markdown中，049负责编辑保存；044只持有阶段和确认基准，不保存第二正文。确认时重读已保存正文及附件版本，构建提交前再次校验；改稿使依赖旧稿的在途阶段任务暂停/失效，旧候选零工程写入。它们不新增V9阶段字段，也不把聊天摘要当当前稿。
