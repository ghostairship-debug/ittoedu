# 1.9：会话恢复、删除与真实课例打磨（延续可见聊天入口）

## 结果与边界

2026-09-07路线调整：本版新增能力暂缓，先完成[1.8三表面整合与S3收口](../../THREE_SURFACE_ARCHITECTURE_INTEGRATION_PLAN.md)。现有聊天和人工能力保持可用，独立当前阻断可在1.8先修；不要求等待完整重写。r19-051的PPTX媒体/效果也增加r18-060依赖，避免新增能力绕过S3。原节点、验收范围及candidate发布语义保留，未新增active状态。
...
延续 Owner 2026-09-07 决定，AI 聊天入口默认显示；本版新增能力仍须对应验收。本版只形成 `v1.9.0-rc.N` engineering candidate 源码标签，不发布 HTML 或安装器；Owner accepted 在 S4（2.0）统一签署。
...
| `r19-051-pptx-media-effects` | 增强内嵌媒体与可表达的简单演示效果 | `r18-051-pptx-editable-diagrams`, `r18-060-release` | 否 | `app-save-recovery`, `store-slide`, `authoring-interaction`, `published-slide`, `export-pptx` | 内嵌且当前媒体管线可解码的视频/音频走现有资产与播放能力；外链不自动下载；仅将已有声明式显隐/入场语义可准确表达的简单触发映射到正式交互，复杂时间线与转场明确静态保留；连续播放、保存重开、离线 HTML 及静态导出提示正确，不声称 PowerPoint 动画等价 |
