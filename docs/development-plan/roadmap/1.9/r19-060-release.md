...
全部1.9真实工作空间/课例、三格式材料、真实文档共编、新Flow/可编辑Word数学、持续创作及PPTX媒体目标完成，形成可复核engineering candidate并在获得发布授权后创建v1.9.0-rc.N源码标签。Owner不要求旧格式兼容；功能与新模型自身的保存/重开/运行仍须验证。
...
1. 确认040–049、050、051完整依赖闭包与证据，复用仍适用的1.8结果；核对真实课例/首存、三格式材料、四阶段文件、双流程/共编/接续、新Flow与Word。050汇合041容器、049文件、044阶段和048Flow的真实组合后才可进入候选收口，不能以早期局部节点完成代替新范围验收。
...
- 反例：遗漏真实课例/首存、三格式之一、文档共编、新Flow、可编辑Word数学、长任务或媒体；仅fixture成功或提前用无后缀正式标签，均不通过。
