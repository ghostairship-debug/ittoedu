...
3. 中断/首存/恢复或切CLI后，按042真实课例与当前工程目标重新观察，并经049/044重读四阶段文件、附件版本和有效确认；首次工程保存不重建课例对话。存在未保存恢复稿/冲突时先处理，不用旧磁盘稿冒充当前稿。复用正式receipt去重，已完成单元不重做；提交后结果未知先核实真实文件/工程，不假定失败重提或补造成功回执。用户纠正不能被旧阶段摘要覆盖。
