...
Flow/Spatial 工具分别依赖 1.3 对应 Chart delivery，必须承载已交付的正文图表块和世界图表。Builder V2 经同一共享图表内容/Surface command 写入，不能因工具范围较旧而遗漏图表，也不能为工具另建一份图表数据模型；依赖只针对能力节点，不要求这些工具等待整个 1.3 发布流程。
...
| `r14-021-flow-tools` | 正式工具覆盖 Flow 正文结构和内嵌内容 | `r14-000-target-wire`, `r14-001-tool-receipt`, `r13-003-flow-chart-delivery` | 否 | `store-flow`, `authoring-flow`, `props-flow` | 通过工具插入、更新、移动、删除 Flow block（含 1.3 正文 Chart）与内嵌对象；canonical location 不因前置插入而漂移；错误 target 和 stale revision 均零写入，一次调用一个历史事务；保存重开、Player 与适用 DOCX 的内容和顺序一致 |
| `r14-022-spatial-tools` | 正式工具覆盖 Spatial object、camera、path 与 relation | `r14-000-target-wire`, `r14-001-tool-receipt`, `r13-004-spatial-chart-delivery` | 否 | `store-spatial`, `authoring-spatial`, `props-spatial` | 通过工具创建对象（含 1.3 world Chart）、设置 camera、编辑 path / relation；非法引用和环路按合同拒绝；一次调用一个历史事务；保存重开后稳定身份、几何、图表数据和关系不变；Player 可完成路径导航 |
