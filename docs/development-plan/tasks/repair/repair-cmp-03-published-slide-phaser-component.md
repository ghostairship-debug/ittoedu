...
- Status / Owner: queued / codex/repair-cmp-03
...
- Write scope / Baseline: planning baseline `7d17fed384804e998e15ae21380ed98259acf897`，active 时改为 RTP-04 集成 SHA；仅允许新建一个 Slide scene-local Published Phaser Component mount，窄改 `src/player/surfaces/publishedComponentMount.ts` 的包解析/API 4 context 共性、`src/player/surfaces/slide/SlidePublishedAdapter.ts` 的 scene 路由和生命周期，以及新增 V9 fixture、unit/integration 与 `tests/e2e/publishedPhaserComponentV2.spec.ts`；禁止修改 Schema、Published producer、App/main/preload、Flow/Spatial/global/shared host、capture/PDF/PPTX、Legacy PlayerApp/renderNode 与 generated 输出。
