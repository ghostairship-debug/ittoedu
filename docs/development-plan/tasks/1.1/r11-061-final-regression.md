...
- Write scope: 产品树只读验证；当前失败只允许修改 `src/renderer/{runtime,interactions}/commit*Authoring.ts` 的 history-entry 端口语义、Published authoring 重挂载的直接责任 Owner、对应最近层测试及必要生成物；代码修复使 060 identity 失效时，允许原子更新 `docs/development-plan/inventories/legacy-consumers.json`；本卡与任务板。
