...
- Retry count / last failure class: `3 / test-oracle: the Nodes duplicate command correctly selected the new Spatial global item and intentionally switched the sidebar from Layers to Properties, so a still-visible Nodes-row assertion targeted an unmounted tab`
