...
- Retry count / last failure class: `4 / test-oracle plus harness budget: saveCurrent intentionally cleared the already-proven rejected-drop alert, so the following stale close-button locator consumed the remainder of an undersized 240-second total timeout`
