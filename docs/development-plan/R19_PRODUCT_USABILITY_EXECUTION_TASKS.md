...
> 2026-09-16 Owner 要求：不由用户逐项操作或派发；任务可并发、高效率、最小检验、次旗舰可执行。本文件细化[原可用性方案第 7 节](R19_PRODUCT_USABILITY_IMPROVEMENT_PLAN.md#7-常规任务完整链路与效率收敛)。2026-09-17 U01–U10 已按本任务包实施并接通，工程证据及未覆盖范围见[执行记录](reviews/2026-09-17-r19-usability-execution.md)；编号是本次实施分包，不是新增路线节点或协调状态。

## 1. 执行方式与共同边界

**当前用途（2026-09-18）：已实施任务的历史设计参考。** U01–U10 不再整体派发。现行规则以[1.9 完整实施方案](R19_FRONTEND_SPECIAL_IMPLEMENTATION_PLAN.md)为准，工作台默认不强制四阶段。以下执行指令只用于理解当时分工，不能因换执行者重新跑一轮，也不能把当时的四次确认写成现行产品规则。

当时的执行分工为：一个 ROOT 连续完成基线准备、派发、集成、补验和结果记录，用户无需逐项派发。本文不授权重做已完成批次。
