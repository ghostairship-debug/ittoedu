...
> 2026-09-16 Owner 要求：不由用户逐项操作或派发；任务可并发、高效率、最小检验、次旗舰可执行。本文件细化[主方案第 7 节](R19_PRODUCT_USABILITY_IMPROVEMENT_PLAN.md#7-常规任务完整链路与效率收敛)。当前仅完成任务设计，U01–U10 均未实施；编号是本次实施分包，不是新增路线节点或协调状态。

## 1. 执行方式与共同边界

后续获得实施指令后，由一个 ROOT 连续完成基线准备、派发、集成、补验和结果记录。用户无需复制任务提示、手动建工作区、分配写锁或逐项确认。已授权范围内的接口细化、例行冲突处理、失败定位和必要重跑由 ROOT 处理；仅新的产品能力取舍、数据语义改变或既有授权无法覆盖的外部动作交回用户。开发连续推进不替代产品内手动创作流程的四次当前稿确认。
