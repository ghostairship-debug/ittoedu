...
当前接手顺序：**总纲第 5.1 节 → [1.9 完整实施方案](R19_FRONTEND_SPECIAL_IMPLEMENTATION_PLAN.md)第 9 节，沿 F00–F08 做到 050/051/060 → 本包合同与源码**。文件名含 FRONTEND_SPECIAL 是历史名称，范围是整个 1.9，不能以前端完成代替版本完成。共享接口先串行固定，再有限并行独立叶子；不需要大集群。旧 U01–U10 已实施，上轮有限收尾已停止，不能从旧文档的将来时重新启动。
...
| [1.9 完整实施方案](R19_FRONTEND_SPECIAL_IMPLEMENTATION_PLAN.md) | 完成 1.9 的唯一当前实施入口：F00–F08 分包、写域、执行顺序、交接与 050/051/060 收口 |
...
| 文档组 | 应如何使用 |
|---|---|
| [共用编辑方案](R19_SHARED_DOCUMENT_EDITOR_IMPLEMENTATION_PLAN.md)及正文／文件合同 | 已有能力的实现依据、技术边界与分项证据；当前派工只看完整实施方案 |
...
| [1.9 路线与验收规格](roadmap/1.9/README.md) | 正式 DAG 与节点规格；实施次序、050/051/060 收口以完整实施方案为准 |
