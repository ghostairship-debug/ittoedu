...
- 2026-09-15更新目标分期：1.8修当前CLI/工程编辑及效率基础；1.9先选择真实工作空间、命名创建独立课例，042负责课例身份及首存连续性，041负责左侧目录/课例对话、中间聊天、右侧标签工作台，045分别完成PDF/DOCX/PPTX课例材料提取与消费，044负责内置自动/手动。046–049实现统一正文合同、连续编辑核心、Flow/可编辑Word数学及真实Markdown共编，全部纳入1.9。自动须采用并成功读取材料；手动分别确认四阶段真实当前文件。上述外部Skill在044实施前继续现有停点，不把计划当guard已改。2.0完成全部软件内QA/修复。执行见[共用编辑方案](docs/development-plan/R19_SHARED_DOCUMENT_EDITOR_IMPLEMENTATION_PLAN.md)与[1.9路线](docs/development-plan/roadmap/1.9/README.md)。
...
- 1.9目标中课例在真实目录创建时即有稳定身份，对话/trace在应用本地按课例与conversation隔离；工程编辑目标仍绑定真实projectId/路径/revision/epoch，首次保存只绑定工程文件，不重建课例对话。工程Save As创建新workspace identity且不复制原工程会话/候选/句柄。四阶段正文与独立材料集保存为课例真实文件，应用记录不保存第二正文；删除聊天/缓存不得删除课例文件或未保存恢复稿，也不承诺删除外部CLI历史。详见[课例与文件合同](docs/development-plan/R19_LESSON_DOCUMENT_WORKSPACE_CONTRACT.md)，当前实现须在对应节点接入后才可称完成。
