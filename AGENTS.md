...
- V9 已有字段、判别器和语义软冻结；additive 可选字段必须独立合同提交并保持 `.strict()`。Table/Chart 是 Owner 明确批准的 V9 新 strict discriminator 窄例外，并在 Published Course V2 增加匹配的严格分支；旧 V9 必须继续可读，旧 reader 遇到新分支必须明确失败，不得静默剥离、截图降级或塞入 legacy SceneNode。
