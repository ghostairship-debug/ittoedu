...
> 计划版本：17.5（2026-08-26：固定候选 `7d17fed` 已通过完整 phase gate；RTP-04 与 LEG-003 在互斥写入范围并行，CMP-03 因共享 Slide Published host 排在 RTP-04 后）
>
...
上一批三个互斥 S2 已按 photosynthesis V9/V2 oracle → Slide API 2 Published playback → 真实 V9 预览联网的顺序合入并通过固定候选 `7d17fed` phase gate；独立 Reviewer 发现并关闭了 Phaser 核心销毁、overlay A→B 授权残留与跨文档迟到 IPC 三类 P1。最新 consumer 审计确认 RTP-04 session-global API 2 Runtime 与 CMP-03 Slide Phaser Component 均 Ready；二者共享 Slide Published host，当前只激活 RTP-04，CMP-03 排队并在 RTP-04 合入后重置 baseline。`f3fd31f` 又以正常 V9 生命周期表征解锁 LEG-003，可在独占 App/main/preload 范围与 RTP-04 并行；网络/CORS/捕获因没有真实作者消费链继续 No-Ready。
...
当前任务板以 RTP-04 与 LEG-003 为 active、CMP-03 为 queued；RTP-04/LEG-003 写入范围互斥，CMP-03 因共享 Slide Published host 等 RTP-04 合入。CMP-03 通过后再分别审计 sample、benchmark/portability/release 的最小 V9/V2 替代，不打包成“大清退”；其余纵切继续按 consumer 证据准入。
