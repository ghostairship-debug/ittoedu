...
> **2026-09-22 方案取代声明：** 当前目标以[果铃 2.0 收敛方案](果铃2.0收敛方案.md)及其执行包为准。旧总纲正文保留为 1.x 历史与源码事实依据，不再是当前路线。 本轮只修订方案，尚未开始新产品实现。
> 当前路线整理日期：2026-09-17。当前任务、数量和状态只看自动生成的 [任务板](docs/development-plan/TASK_BOARD.md)。
>
...
### 5.1 当前阶段：2.0 轻量上下文共编与内部生产验收

2026-09-21：1.9 已在 Owner 代验授权下完成剩余实机和工程门禁并接受为内部源码 RC `v1.9.0-rc.1`，以[签收包 §⑫](docs/development-plan/reviews/2026-09-19-r19-acceptance-package.md)覆盖其中过时的阶段失败表，不重开已完成的 1.9 任务。当前实施入口为[1.9.1–2.0上下文共编方案](docs/development-plan/R19_1_TO_R20_CONTEXTUAL_AUTHORING_PLAN.md)与[2.0正式DAG](docs/development-plan/roadmap/2.0/README.md)，协调状态只看任务板。

当前形态：真实工作空间＋可选文件夹项目，会话归目录，打开文件仅改变编辑焦点。单一自动目标提示；课件默认当前页、点选转当前选择、发送固定版本与目标。材料通过路径/@/附件/粘贴使用，默认按任务推进，仅用户要求先审真实制品时停点。聊天可选择/复制，/与@调用实际能力，项目/会话管理简化；内容按需打开、四向布局。工作台轻改与编辑器模式精修共用工程/历史，保留场景/状态两层，不恢复简洁/专业模式。Office本阶段走系统宿主。

上下文共编已接入 Markdown 与 Flow：选区形成会话态目标，浮现语义上下文卡，人工与 AI 卡片/聊天共用冻结目标、候选校验和正式历史；不新增持久化 paragraph 对象。AI 指令由用户自己提出，常用格式复用现有正文工具，不建设固定 AI 功能菜单。

共享接口由唯一集成人顺序接线；独立叶子在精确非重叠写域下并行。设置、首次外部处理说明、内置方法、图片材料与应用记录管理已汇入实际界面，后续围绕完整生产链及确有缺口的验收推进，不重复建设编辑器或原生模型循环。

#### 已有工程成果与验收缺口

| 范围 | 当前事实与入口 |
|---|---|
| 1.9 基线 | 内部 RC 已签收；文件共编、Flow／Word、材料与会话的未受影响证据继续复用 |
| 上下文共编 | 选区/观察/映射单元链在本候选实跑绿，**只认届时命令输出、不固化计数**（2026-09-21 16:02–16:07 +08:00 复测）：`npx vitest run tests/unit/flowContextSelection.test.ts tests/unit/courseChatObservation.test.ts tests/unit/courseChatPanel.test.tsx tests/unit/flowSharedAuthoringAdapters.test.tsx --maxWorkers=1` → `Tests 124 passed (124)`；`npx vitest run tests/unit/markdownSourceMap.test.ts tests/unit/documentEditPreview.test.ts tests/unit/documentAiTaskController.test.ts tests/unit/lessonDocumentAiTask.test.ts tests/unit/sharedDocumentMarkdown.test.ts --maxWorkers=1` → `Tests 79 passed (79)`；`npx vitest run tests/unit/externalAiNoticeRepository.test.ts tests/unit/useExternalAiNotice.test.tsx tests/unit/nativeAgentDiagnostics.test.tsx tests/unit/nativeAgentHelp.test.tsx tests/unit/nativeAgentQuestion.test.tsx tests/unit/lessonConversationChat.test.tsx --maxWorkers=1` → `Tests 42 passed (42)`。用例数随批次变化，**不得把任何一次的数字当常量引用**；Markdown 与 Flow 的**真实选区 Luna 用例受门控未运行**，且 Markdown 三 CLI 不是一条无返工的链（合并运行 failed，opencode 改过定位符并重跑多次后才通过） |
| 2.0 已有分项 | 原生课件三 CLI、PPTX 7 项、固定 HTML 1 项、原生矩阵 3 项与跨文档共编 3 项在本候选上**不可追溯**。`output/playwright/r20-*` 共 19 个目录，**不能用通配一概而论**；2026-09-21 16:06 +08:00 实测：`r20-pptx`、`r20-fixed-html`、`r20-native-matrix` 三个目录里只有一个 45 字节 `.last-run.json`（`{"status":"passed","failedTests":[]}`）；`r20-coauthor-preservation` 的 `.last-run.json` 同为 45 字节，但同目录另有 2 张共 844660 字节截图；`r20-native-codex`、`r20-native-claude`、`r20-native-opencode` 各 4 个文件、约 2.35 MB，**没有** `.last-run.json`，且被 `tests/e2e/stabilizationCoreUsability.spec.ts:945` 与 `:1010` 引用为证据目录；`r20-flow-selection` 有 168 个文件 / 14235613 字节、`r20-real-other-cli` 有 88 个文件 / 3701947 字节，二者 `.last-run.json` 均为 `{"status":"failed"}`。复测命令：`Get-ChildItem output/playwright -Directory -Filter 'r20-*' | ForEach-Object { $f = Get-ChildItem $_.FullName -Recurse -File; "$($_.Name) files=$($f.Count) bytes=$(($f | Measure-Object Length -Sum).Sum)" }`。保全 27 项是 automated pass，**不是 Owner accepted**。见[本轮记录](docs/development-plan/reviews/2026-09-21-r20-contextual-acceptance.md) |
| 真实生产验收 | 020/040 尚需同一完整课件从材料到持续修改、QA 修复及最终离线交付；025 指定插件对照及030实际读屏尚未闭合 |
| S4 与发布 | Owner 已授权代验；仍须满足真实门禁后冻结同一源码与固定 HTML，当前不创建正式 `v2.0.0` |

1.8 Owner S3、1.9 内部 RC 和2.0 S4分别保留，文档更新不晋升。PDF/DOCX/PPTX 各自的材料消费、正文/Flow/Word/文件共编及媒体继续保全。默认任务流程取代固定四阶段要求；用户明确批准的制品仍绑定真实当前版本。2.0完整软件内QA/修复按正式DAG验收，长期Runtime位置编辑和持续AI深化不自动扩成当前任务。

高频准确编辑与复杂批量／交互编辑分别验收；模型首轮表现与包含返工的最终结果、时间分列。原生 CLI 能力、唯一事务、三 Surface、保存恢复及既有导出保持；不因 UI 改造重复性能实验或模型矩阵。旧证据只在相关实现、依赖或环境变化时重验。

根目录[长期产品方案](果铃_AI原生文件与内容工作台_产品方案_V2.0.md)用于方向与候选研究，不能覆盖已确认前端或充当当前开发清单。旧总纲 5.1 的多轮过程已移至[历史快照](docs/archive/2026-09-planning/2026-09-17-development-route-history.md)，不再作为开工入口。
...
- **1.9 连续创作工作流**：目录会话/文件目标与首存、三格式材料直达、按任务创作/明确审稿、自然聊天与两种编辑位置、恢复接续；统一新Flow正文、真实Markdown共编及可编辑Word数学同步交付，不保留本次旧格式兼容或迁移。
