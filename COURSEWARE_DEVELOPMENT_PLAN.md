...
### 5.1 当前阶段：1.9 教师可用性与前端专项

2026-09-17 Owner 决定：完成 1.9 验收前，先按已确认 V3.1 集中完成前端和教师操作流程，避免继续零散修补布局。唯一实施入口为[前端专项方案](docs/development-plan/R19_FRONTEND_SPECIAL_IMPLEMENTATION_PLAN.md)，视觉与交互依据为[设计说明](双形态UI设计/00-设计说明.md)。目前专项方案完成，产品实施未开始；实际派工只看任务板。

当前确定的形态：工作空间为真实根目录；项目为可选真实文件夹；根目录资源在左栏上方，项目与会话在下方；允许不建项目直接工作；画布按需打开且布局可调。工作台保留高频编辑，原生编辑器共用同一工程与历史、取消简洁／专业模式，并保持场景／状态两层管理。Office 本阶段用系统宿主打开，原格式编辑引擎和外部 HTML 编辑不进入本专项。

实施方式为一个 ROOT 负责共享接口和集成，接口稳定后最多 2–3 个独立叶子并行；工具不支持隔离或写域相交时直接串行。先打通“真实工作空间→MD 编辑保存→调整布局→重开”，再完成项目会话、材料创作和原生编辑器链路。具体写域、检查和停止条件只在专项方案维护。
...
1.8 Owner S3、1.9 engineering candidate 和 2.0 S4 的证据及签署边界仍分别有效，不因文档整理晋升。自动流程必须采用并实际读取材料；手动分别确认教学简报、策划、呈现简报、脚本的真实当前文件。2.0 的软件内检查／修复闭环保留为后续范围，Runtime 位置编辑和持续 AI 深化不作为本次前置。
