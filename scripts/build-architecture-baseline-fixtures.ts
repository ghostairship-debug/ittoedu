...
/**
 * The single business instant recorded inside `project.json` (`createdAt`,
 * `updatedAt`, `importedAt`) and echoed by the manifest. It stays a UTC ISO
 * string so the recorded semantics are unambiguous.
 */
export const ARCHITECTURE_BASELINE_FIXTURE_MTIME = '2026-08-24T00:00:00.000Z'

/**
 * The ZIP framing timestamp for both archive layers. It is deliberately *not*
 * {@link ARCHITECTURE_BASELINE_FIXTURE_MTIME}: fflate derives the DOS timestamp
 * from local calendar fields, so an absolute instant would emit different bytes
 * in every timezone. See {@link createTimezoneStableZipMtime} for the full
 * rationale — including why the helper, not this module, owns the assertion that
 * the chosen wall clock survives the host's DST rules.
 */
export const ARCHITECTURE_BASELINE_FIXTURE_ZIP_MTIME = createTimezoneStableZipMtime(
...
export interface ArchitectureBaselineFixtureManifest {
  ...
  /**
   * The business instant recorded inside the fixtures, not the ZIP header
   * timestamp — those are deliberately separate; see
   * {@link ARCHITECTURE_BASELINE_FIXTURE_ZIP_MTIME}.
   */
...
// Use the product encoder so zlib and chunk checksums cannot drift from pixels.
// The original damaged bytes are retained only in image-validation negative fixtures.
const PNG_BYTES = encodeImageTransformPng({
  width: 1, height: 1, data: Uint8Array.from([255, 0, 0, 255]),
})
...
function evidencePanelComponent(): {
   ...
   *
   * The inner package must use the same timezone-stable ZIP timestamp as the
   * outer `.h5lesson`: its bytes feed `sha256` into `project.json`, so any drift
   * here also changes the outer archive.
   */
