...
/**
 * ZIP entries carry a DOS timestamp, and fflate derives it from the *local*
 * calendar fields of the supplied date (`getFullYear()` … `getSeconds()`).
 * Handing it an absolute instant such as {@link ARCHITECTURE_BASELINE_FIXTURE_MTIME}
 * therefore writes different header bytes in every timezone, which cascades
 * through the inner `.h5component` bytes, its provenance `sha256`, and the outer
 * `.h5lesson` DEFLATE output.
 *
 * A date built *from* local fields keeps those fields identical in every
 * timezone, so the DOS timestamp — and with it every generated byte — is stable.
 * Noon avoids the midnight-adjacent DST shifts that could renormalise the wall
 * clock. This value is ZIP framing only; business fields never use it.
 */
export const ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS = {
  year: 2026,
  month: 8,
  day: 24,
  hours: 12,
...
export const ARCHITECTURE_BASELINE_FIXTURE_ZIP_MTIME = new Date(
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.year,
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.month - 1,
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.day,
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.hours,
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.minutes,
  ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS.seconds,
  0,
)

/**
 * Fails the build instead of emitting timezone-dependent bytes if the host
 * renormalises the chosen wall clock (only reachable in an exotic DST rule).
 */
function assertZipMtimeIsTimezoneStable(): void {
  const fields = ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS
  const actual = ARCHITECTURE_BASELINE_FIXTURE_ZIP_MTIME
  const observed = {
    year: actual.getFullYear(),
    month: actual.getMonth() + 1,
    day: actual.getDate(),
    hours: actual.getHours(),
    minutes: actual.getMinutes(),
    seconds: actual.getSeconds(),
  }
  for (const [key, expected] of Object.entries(fields)) {
    if (observed[key as keyof typeof observed] !== expected) {
      throw new Error(
        `ZIP timestamp is not timezone stable in ${
          process.env.TZ ?? 'the host timezone'
        }: expected local ${JSON.stringify(fields)}, observed ${JSON.stringify(observed)}`,
      )
    }
  }
}
...
export interface ArchitectureBaselineFixtureManifest {
   ...
   * {@link ARCHITECTURE_BASELINE_FIXTURE_ZIP_LOCAL_FIELDS}.
   */
...
export function buildArchitectureBaselineFixtureOutputs(): ArchitectureBaselineFixtureBuild {
  assertZipMtimeIsTimezoneStable()
