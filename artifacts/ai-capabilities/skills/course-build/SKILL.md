...
输入：用户当前任务与已有真实文件。若用户已确认当前策划和脚本则使用它们；普通任务不因缺少四阶段文件而停下。按片段保留教学作用、布局、讲解、操作与前后路径；输出正式工具多步候选，前序新对象只用 created-item/created-scope 引用。交付摘要注明哪些视觉与互动还需实际宿主检查，不宣称教师已验收。
