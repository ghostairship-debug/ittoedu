import { z } from 'zod'
import { lessonAgentWorkspaceSchema } from './workspaceIdentity'
...
export const lessonDocumentAiRequestSchema = z.discriminatedUnion('operation', [
  z.object({ operation: z.literal('start'), workspace: lessonAgentWorkspaceSchema, ref, baseVersion: version, ranges: z.array(documentAiRangeSchema).min(1), epoch: z.number().int().nonnegative(), adapter: localAgentIdSchema, instruction: z.string().min(1).max(20000) }).strict(),
  z.object({ operation: z.literal('read'), workspace: lessonAgentWorkspaceSchema, taskId: z.uuid() }).strict(),
  z.object({ operation: z.literal('stop'), workspace: lessonAgentWorkspaceSchema, taskId: z.uuid() }).strict(),
])
