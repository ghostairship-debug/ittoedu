...
export const aiTaskIdentityFields = { taskId: z.uuid(), epoch: z.number().int().nonnegative(), workspace: aiWorkspaceIdentitySchema }
...
export const aiUserInputSchema = z.discriminatedUnion('kind', [
  z.object({ ...input, kind: z.literal('extend-budget'), minutes: z.literal(20) }).strict(),
...
export const aiInputDeliverySchema = z.object({
  ...
  /** Present only after the task owner durably accepts an explicit extension. */
