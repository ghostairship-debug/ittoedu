...
export const lessonDocumentRequestSchema = z.discriminatedUnion('operation', [
  ...
  z.object({ operation: z.literal('read-ai-records'), ref }).strict(),
  z.object({ operation: z.literal('clear-ai-records'), ref, ids: z.array(z.string().min(1)) }).strict(),
  z.object({ operation: z.literal('invalidate'), ref }).strict(),
  ...
  z.object({ operation: z.literal('prepare'), ref, ranges: z.array(edit), epoch: z.number().int().nonnegative() }).strict(),
  z.object({ operation: z.literal('apply'), request: z.object({ ref, baseVersion: version, epoch: z.number().int().nonnegative(), operationId, edits: z.array(edit) }).strict() }).strict(),
  z.object({ operation: z.literal('revert'), record, currentVersion: version }).strict(),
