...
export interface CourseAgentCapabilityVariant {
  operation?: string
  mode?: string
  scopes: string[]
  target?: string
...
export interface CourseAgentCapabilityEntry {
  ...
  dependencies?: string[]
  available?: boolean
  variants?: CourseAgentCapabilityVariant[]
}
...
export interface CourseAgentCapabilityCardOptions {
  ...
  mode?: string
  nativeType?: string
  surface?: CourseAgentCapabilityQuery['surface']
  owner?: CourseAgentCapabilityQuery['owner']
  carrier?: CourseAgentCapabilityQuery['carrier']
}
...
function validateQuery(data: CourseAgentCapabilityData, input: CourseAgentCapabilityQuery): void {
  ...
  for (const key of ['operation', 'mode', 'nativeType'] as const) if (input[key] !== undefined && (typeof input[key] !== 'string' || !input[key]!.trim())) throw new Error('能力卡选择无效')
...
function matchingVariants(entry: CourseAgentCapabilityEntry, query: CourseAgentCapabilityCardOptions) {
  const scopesMatch = (scopes: string[]) => scopes.some(scope => (!query.surface || scope.startsWith(`${query.surface}:`)) && (!query.owner || scope.endsWith(`:${query.owner}`)))
...
function discriminator(root: Record<string, any>, branch: unknown, field: string): string[] | undefined {
  ...
  return Array.isArray(value?.enum) && value.enum.every((item: unknown) => typeof item === 'string') ? value.enum : undefined
}
function schemaSelection(value: unknown, selectors: Record<string, string>, root: Record<string, any>): unknown {
  ...
  for (const union of ['anyOf', 'oneOf']) {
    ...
    for (const [field, wanted] of Object.entries(selectors)) {
      ...
      const branches = result[union].filter((branch: any) => discriminator(root, branch, field)?.includes(wanted))
...
export function readCourseAgentCapability(data: CourseAgentCapabilityData, id: string, options: CourseAgentCapabilityCardOptions = {}) {
  ...
  if (Object.keys(options).length) for (const field of ['operation', 'mode'] as const) {
  ...
  if (content.examples && Object.keys(selectors).length) {
    content.examples = content.examples.filter((example: any) => Object.entries(selectors).every(([field, wanted]) => example[field] === undefined || example[field] === wanted))
    if (options.nativeType && content.examples.some((example: any) => example.nativeType === options.nativeType)) content.examples = content.examples.filter((example: any) => example.nativeType === options.nativeType)
  }
  return { version: 1 as const, semanticVersion: data.semanticVersion, entry: structuredClone(entry.variants ? { ...entry, scopes: content.supportedScopes,
...
export function runCourseAgentCapabilityQuery(data: CourseAgentCapabilityData, args: readonly string[]) {
  ...
  const query: CourseAgentCapabilityQuery = { detail: 'full' }
