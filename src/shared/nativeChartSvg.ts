...
export function buildNativeChartSvg(chart: NativeChartViewContent, width: number, height: number, id = ''): string {
  ...
  if (view.cartesianSeries && (chart.chartType === 'bar' || chart.chartType === 'line' || chart.chartType === 'area')) {
    if (chart.style.showGridLines) for (const line of view.gridLines ?? []) content += element('line', { x1: area.x, y1: line.y, x2: area.x + area.width, y2: line.y, stroke: '#e5e7eb', 'stroke-width': 1 })
    if (chart.style.showValueAxis) for (const tick of view.gridLines ?? []) content += text(tick.value, { x: area.x - 8, y: tick.y + 4, 'text-anchor': 'end', 'data-chart-value-tick': 'true' })
    let plot = ''
    for (const series of view.cartesianSeries) {
      ...
      for (const point of series.points) {
        if (chart.style.showDataLabels && point.y >= area.y && point.y <= area.y + area.height) {
          const bar = series.bars?.find(item => item.categoryId === point.categoryId)
          plot += text(point.value, { x: bar ? bar.x + bar.width / 2 : point.x, y: Math.max(area.y + 12, point.y - 7), 'text-anchor': 'middle', 'data-chart-data-label': 'true' })
        }
    ...
    if (chart.style.showCategoryAxis) for (const category of view.categories ?? []) content += text(category.label, { x: category.x + category.width / 2, y: area.y + area.height + 18, 'text-anchor': 'middle', 'data-chart-category-id': category.id })
  }
