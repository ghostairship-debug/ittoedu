...
const common = '只根据本轮不可变快照和正式工具输入 Schema 工作。只返回结构化候选或讨论文本；不直接修改 .h5lesson、Store、源码或其他会话。遵循 Native → Recipe → Existing Component → Generated Component → Runtime；选择高阶载体必须说明低阶能力不能满足的具体需求。候选检查与宿主提交是不同状态，不把 CLI 原生工具成功说成工程已修改。'
