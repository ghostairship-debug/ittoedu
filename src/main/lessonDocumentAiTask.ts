...
export class LessonDocumentAiTasks {
  ...
  async invalidate(scope: LessonRecordsInvalidationScope) {
    const affected = [...this.runs.values()].filter(run => 'all' in scope || (run.input.workspace.lessonId === scope.lesson.lessonId && path.resolve(run.input.workspace.normalizedDirectory).toLowerCase() === path.resolve(scope.lesson.normalizedDirectory).toLowerCase() && (!scope.conversationId || run.input.workspace.conversationId === scope.conversationId)))
  ...
  async operate(raw: LessonDocumentAiRequest): Promise<LessonDocumentAiResult> {
    ...
    if (input.operation === 'start') {
      if (input.workspace.lessonId !== input.ref.lessonId || path.resolve(input.workspace.normalizedDirectory).toLowerCase() !== path.resolve(input.ref.lessonDirectory).toLowerCase()) throw new Error('文件不属于当前课例')
      ...
      const result: LessonDocumentAiResult = { taskId, sessionId: reply.sessionId, status: 'running', message: `正在修改 ${input.ref.relativePath}` }
