...
export function LessonConversationNavigation(props: LessonConversationNavigationProps) {
  ...
  const [pendingDelete, setPendingDelete] = useState<LessonConversation | 'owner' | 'all' | null>(null)
  ...
  useEffect(() => { setQuery(''); setMatches(null); setPendingDelete(null); setError(null) }, [scopeKey])
  useEffect(() => {
    let live = true
  ...
  async function remove() {
    ...
    await run(async captured => {
      const result = await captured.operation(scope === 'all' ? { operation: 'delete-all-application-records' }
        : { operation: 'delete-conversation', owner: captured.owner, ...(scope === 'owner' ? {} : { conversationId: scope.conversationId }) })
      if (activeKey.current !== scopeKey) return
  ...
  return <section className="lesson-conversation-navigation" aria-label={sectionLabel}>
    ...
    <details className="lesson-conversation-records"><summary>管理对话记录</summary><button type="button" disabled={busy} onClick={() => setPendingDelete('owner')}>{isLessonOwner ? '删除本课例对话记录' : props.owner.kind === 'workspace' ? '删除本工作空间对话记录' : '删除本项目对话记录'}</button><button type="button" disabled={busy} onClick={() => setPendingDelete('all')}>删除全部应用对话记录</button></details>
    {pendingDelete && <div role="dialog" aria-label="确认删除对话记录">
      <p>{pendingDelete === 'all' ? '删除全部课例和独立工程的应用对话、任务与日志？' : pendingDelete === 'owner' ? `删除“${ownerName(props.owner, props.ownerLabel)}”的全部对话、任务与日志？` : `删除“${pendingDelete.title}”的对话、任务与日志？`}</p>
      <p>{deleteNotice}</p>
