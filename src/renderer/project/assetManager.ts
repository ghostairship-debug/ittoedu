...
export function createImageAssetImport(input: Pick<SelectedImageResult, 'name' | 'mimeType' | 'bytes'>, options: ImportImageOptions = {}): ImportedImageAsset {
  const asset = createImageAssetMetadata(input, options)
  const url = options.blobUrlRegistry?.create(`asset:${asset.meta.id}`, asset.bytes, input.mimeType)
  return { ...asset, ...(url === undefined ? {} : { url }) }
}
