...
export function ProductivityDialog({ getContext, getAssetFiles, onCommit, onClose }: ProductivityDialogProps) {
  const [mode, setMode] = useState<'text' | 'color' | 'clone'>('text')
  ...
  return <div className="modal-backdrop"><section role="dialog" aria-modal="true" aria-label="设计生产力" style={{ background: 'var(--panel-bg, #20242c)', color: 'inherit', padding: 24, borderRadius: 12, width: 'min(880px, 92vw)', maxHeight: '88vh', overflow: 'auto' }}>
    <h2>设计生产力</h2>
    <label>操作 <select aria-label="生产力操作" value={mode} onChange={e => { setMode(e.target.value as typeof mode); reset() }}><option value="text">批量查找替换</option><option value="color">应用项目色</option><option value="clone">克隆参考页</option></select></label>
    {mode === 'clone' ? <><p>复制本工程的演示页及其可编辑内容；生成独立对象、状态和素材，一次撤销恢复。</p><label>参考页 <select aria-label="参考页" value={sceneId} onChange={e => setSceneId(e.target.value)}><option value="">选择参考页</option>{context.document.surfaces.flatMap(s => s.type === 'slide' ? s.scenes.map(scene => <option key={scene.id} value={scene.id}>{s.title} / {scene.name}</option>) : [])}</select></label></> : <>
    ...
    <footer style={{ display: 'flex', gap: 12, marginTop: 20 }}><button type="button" onClick={commit} disabled={mode === 'clone' ? !sceneId : !preview || !selected.length}>{mode === 'clone' ? '克隆参考页' : '应用勾选项'}</button><button type="button" onClick={onClose}>取消</button></footer>
  </section></div>
}
