...
function TableField({ context, field, columnId, value, label }: {
  context: FlowPropertiesContext
  field: 'table-caption' | 'table-header'
  columnId?: string
  value: string
  label: string
}) {
  const composing = useRef(false)
  const edit = context.textEdit
  const matches = edit?.field === field && edit.tableColumnId === columnId
  const text = matches ? (edit.draft as { text: string }).text : value
  return <label className="form-field"><span>{label}</span><input className="form-input" aria-label={label} value={text}
    onFocus={() => context.commands.beginTableFieldEdit(field, columnId)}
    onChange={event => context.commands.updateTableFieldDraft(field, columnId, event.currentTarget.value, composing.current)}
    onCompositionStart={() => { composing.current = true; context.commands.updateTableFieldDraft(field, columnId, text, true) }}
    onCompositionEnd={event => { composing.current = false; context.commands.updateTableFieldDraft(field, columnId, event.currentTarget.value, false) }}
    onBlur={() => { if (!composing.current) context.commands.finishTableFieldEdit() }}
    onKeyDown={event => {
      if (composing.current || event.nativeEvent.isComposing) return
      if (event.key === 'Escape') { event.preventDefault(); context.commands.finishTableFieldEdit(true) }
      if (event.key === 'Enter') { event.preventDefault(); context.commands.finishTableFieldEdit() }
    }}
  /></label>
}
...
export function FlowTableProperties({ context, table }: { context: FlowPropertiesContext; table: FlowTableBlock }) {
  ...
  return <section className="property-section" data-testid="flow-table-properties">
    ...
    {table.columns.map((column, index) => <div key={column.id}>
      <TableField context={context} field="table-header" columnId={column.id} value={column.header} label={`第 ${index + 1} 列表头`} />
