...
export const runtimeSourceTool: AuthoringToolDefinition<z.infer<typeof schema>> = {
  ...
  async plan({ document, destination, value, resources, signal }) {
    ...
    if (result.status === 'no-op' && !value.observeButton) return { transaction: { projectId: document.id, baseRevision: document.revision, nextDocument: document, resourceChanges: {} }, affected: [] }
    const behaviorEvidence: DynamicBehaviorObservation[] = []
    const nextDocument = result.status === 'no-op' ? document : result.plan.nextDocument
    await admitDynamicCandidate(nextDocument, resources, [{ locationId: target.locationId, stateId: target.stateId, instanceIds: [destination.target.itemId] }], signal, false, {
      ...(value.observeButton ? { buttonCheck: { ...value.observeButton, instanceId: destination.target.itemId } } : {}),
      onBehaviorEvidence: evidence => behaviorEvidence.push(...evidence) })
    if (result.status === 'no-op') return { transaction: { projectId: document.id, baseRevision: document.revision, nextDocument: document, resourceChanges: {} }, affected: [], behaviorEvidence }
