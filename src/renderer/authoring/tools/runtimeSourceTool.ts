...
export const runtimeSourceTool: AuthoringToolDefinition<z.infer<typeof schema>> = {
  ...
  async plan({ document, destination, value, resources, signal }) {
    ...
    await admitDynamicCandidate(result.plan.nextDocument, resources, [{ locationId: target.locationId, stateId: target.stateId, instanceIds: [destination.target.itemId] }], signal)
