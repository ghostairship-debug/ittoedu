...
type TargetLocation = { locationId: string }
...
const composeTriggerSchema = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('state-enter'), state: reference }).strict(),
  z.object({ kind: z.literal('presenter'), command: z.enum(['next', 'previous']) }).strict(),
])

const composeConditionSchema = z.object({
  inStates: z.array(reference).min(1).max(256).optional(),
  courseState: z.union([
...
function resolveNode(document: CourseProjectDocument, target: TargetLocation, ref: string, what: string): LayerItem {
  ...
  if (matches.length > 1) throw new Error(`${what}“${ref}”匹配到多个同名图层，请改用图层 id`)
  throw new Error(`${what}“${ref}”在当前位置不存在；请先创建该图层或使用已有图层的 id / 唯一名称`)
}

function resolveState(scene: SlideSceneDocument, ref: string, what: string): string {
  ...
  if (byName.length > 1) throw new Error(`${what}“${ref}”匹配到多个同名呈现状态，请改用状态 id`)
  throw new Error(`${what}“${ref}”在当前场景不存在；可用状态：${states.map((state) => state.name).join('、') || '无'}`)
}
...
function composeTrigger(document: CourseProjectDocument, target: TargetLocation, scene: SlideSceneDocument, input: ComposeInput['trigger']): InteractionTrigger {
  switch (input.kind) {
    case 'click': {
      const item = resolveNode(document, target, input.node, '点击目标')
    ...
    case 'input-submit': {
      const item = resolveNode(document, target, input.node, '提交目标')
    ...
    case 'state-enter': return { type: 'presentation.enter', stateId: resolveState(scene, input.state, '触发状态') }
...
function composeConditions(document: CourseProjectDocument, scene: SlideSceneDocument, when: ComposeInput['when']): InteractionCondition[] {
  ...
  if (when.inStates) conditions.push({ type: 'presentation.in', stateIds: when.inStates.map((ref) => resolveState(scene, ref, '条件状态')) })
  if (when.courseState) {
...
function composeActions(document: CourseProjectDocument, target: TargetLocation, scene: SlideSceneDocument, ruleId: string, effects: ComposeInput['effects']): InteractionActionStep[] {
  ...
  effects.forEach((effect, index) => {
    ...
    switch (effect.kind) {
      ...
      case 'hide': {
        ...
        for (const ref of effect.nodes) {
          const item = resolveNode(document, target, ref, '显隐目标')
      ...
      case 'set-state':
        push({ type: 'presentation.set', stateId: resolveState(scene, effect.state, '目标呈现状态') }, 'after-previous')
        break
      ...
      case 'go-to-scene': {
        ...
        const targetStateId = effect.state === undefined ? undefined : resolveState(destination, effect.state, '目标场景状态')
...
export const slideInteractionTool: AuthoringToolDefinition<z.infer<typeof slideInteractionToolInputSchema>> = {
  ...
  description: '仅 Slide scene owner。常用“点击/输入提交/翻页 → 显示或隐藏解释、切换呈现状态（图形与文字随状态同步变化）、设置分数、进入下一步或下一场景”用 compose：触发与目标可给图层/状态/场景的 id 或唯一名称，效果按顺序给出，宿主生成一致引用、步骤 id、分组与收尾导航，并在边界明确拒绝 Published 不支持的组合。导航语义：next-step 先走完当前页剩余呈现步骤再进入下一场景；next-scene 直接跳过剩余步骤进入下一场景（跨 Slide/Flow/Spatial 位置复用课程顺序）；go-to-scene 精确进入指定 Slide 场景。需要延迟、动画/媒体触发或完整条件组合时用 insert/replace（本卡完整 rule Schema，不含最外层 id；第一动作 start 必须 after-previous，导航动作必须最后）。',
    ...
    const result = value.operation === 'insert' ? addSlideSceneInteractionRule(session, { ...value.rule, id: itemId }, options)
      : value.operation === 'compose' ? addSlideSceneInteractionRule(session, (() => {
        const composed: InteractionRule = {
          ...
          actions: composeActions(document, target, scene, itemId, value.effects),
        }
