...
const SLIDE_REJECT_WRONG_OWNER = 'wrong-owner'
export interface SlideClipboardSource { readonly scope: string; readonly history: { readonly present: CourseProjectDocument }; readonly selection: { readonly locationId: string; readonly stateId: string | null } }
...
function sceneLayers(session: SlideClipboardSource) {
...
export function mutatePasteSlideSceneClipboard(
  ...
  for (const entry of clipboard.items) {
    ...
    remapDuplicatedInputState(draft, duplicate, stateKeys, ruleIds)
