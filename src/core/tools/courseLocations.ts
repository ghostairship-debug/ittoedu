...
import { LAYER_REJECT_STALE_REVISION, rejectIfStaleDocument } from './globalLayers'
import { commitCourseProjectMutation as commitSlideProjectMutation } from './courseProjectMutation'
import { mutateDuplicateSlideScene } from './slideStructure'
...
export function duplicateCourseLocation(
  ...
  const stale = rejectIfStaleDocument(project, input.expectedRevision)
  if (stale) return fail(stale.reason ?? LAYER_REJECT_STALE_REVISION, project)
  try {
    const prior = new Set(project.locations.map(location => location.id))
    const next = mutateDuplicateSlideScene(project, location.surfaceId, location.sceneId, input.now)
    const created = next.locations.find(location => !prior.has(location.id))
    if (!created) throw new Error('复制没有返回新课程位置')
    return succeed(next, created.id)
  } catch (error) { return fail(error instanceof Error ? error.message : '复制失败', project) }
}
...
/** Canonical surface title write shared with manual Flow page rename. */
export function renameCourseSurface(project: CourseProjectDocument, surfaceId: string, name: string): CourseProjectDocument {
  const title = courseSurfaceTitleSchema.parse(name)
  const surface = project.surfaces.find(surface => surface.id === surfaceId)
  if (!surface) throw new Error('表面不存在')
  if (surface.title === title) return project
  return commitSlideProjectMutation(project, draft => { draft.surfaces.find(surface => surface.id === surfaceId)!.title = title })
}
