...
import { makeAuthoringAddress, type AuthoringAddressParts, type AuthoringCarrier } from '../../shared/authoringAddress'
import { getEffectiveLayerOrder, isCourseLayerVisibleAtLocation } from '../../shared/courseProjectModel'
import { resolveEffectiveGlobalLayerPlanes } from '../../shared/courseLayerComposition'
import type { CourseProjectDocument, GlobalLayerEntry, GlobalLayerPlane, LayerItem, LocationVisibility } from '../../shared/courseProjectTypes'
...
import { sortScopedLayerList, visitAllCourseLayerItems } from './layerOrder'

export const LAYER_REJECT_LOCKED = 'locked'

export const LAYER_REJECT_STALE_REVISION = 'stale-revision'

export const LAYER_REJECT_WRONG_OWNER = 'wrong-owner'

export const LOCKED_WRITE_REASON = '图层已锁定，除解锁外不能修改。'

export const CONTROLLER_MOVE_REASON = '教师控制器必须留在全局层，不能移动到页面或世界层。'

export const CONTROLLER_DUPLICATE_REASON = '教师控制器不能重复，全课只需一个。'

export const CONTROLLER_PLANE_REASON = '教师控制器固定在全局 Overlay，不能放到 Underlay。'

export const CROSS_OWNER_REORDER_REASON = '不能跨来源假排序。请在同一来源内调整层级。'

export const CROSS_GLOBAL_PLANE_REORDER_REASON = '全局 Underlay 与 Overlay 不能通过排序互换；请先在属性中切换图层位置。'

export const INVALID_GLOBAL_REORDER_REASON = '全局平面排序必须包含该平面的全部图层，且不能混入其他来源。'
...
export function isTeacherControllerLayerItem(
  item: { readonly kind: string; readonly role?: string } | undefined,
...
export function runDocumentMutation(
  document: CourseProjectDocument,
...
export function sameVisibility(left: LocationVisibility, right: LocationVisibility): boolean {
