...
export function assertSceneScope(target: SlideInteractionTarget): void {
...
export function assertRuleTargetsUnlocked(
...
export function locateSceneInteractions(project: CourseProjectDocument, locationId: string): InteractionRule[] {
...
export function validateRule(rule: InteractionRule): void {
...
export function planAddSlideInteractionRule(project: CourseProjectDocument, target: SlideInteractionTarget, rule: InteractionRule, now?: string): CourseProjectDocument {
  assertSceneScope(target)
...
export function planUpdateSlideInteractionRule(project: CourseProjectDocument, target: SlideInteractionTarget, ruleId: string, patch: Partial<Omit<InteractionRule, 'id'>>, now?: string): CourseProjectDocument {
  assertSceneScope(target)
...
export function planDeleteSlideInteractionRule(project: CourseProjectDocument, target: SlideInteractionTarget, ruleId: string, now?: string): CourseProjectDocument {
  assertSceneScope(target)
  const current = locateRule(project, target, ruleId)
  if (!current) throw new Error(emptyRuleScopeMessage())
  assertRuleTargetsUnlocked(project, current)
